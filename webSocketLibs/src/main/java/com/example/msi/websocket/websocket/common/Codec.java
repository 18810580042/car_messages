...
import com.example.msi.websocket.websocket.response.ChildResponse;
import com.example.msi.websocket.websocket.response.Response;
...
public class Codec {
    ...
    public static Response decoder(String text) {
        ...
        if (element.isJsonObject()) {
            ...
            response.setRespEvent(decoderInt(obj, "req_event"));
            response.setAction(decoderStr(obj, "action"));
            response.setSeqId(decoderStr(obj, "seq_id"));
            response.setResp(decoderStr(obj, "req"));
            return response;
        }
        return response;
    }
